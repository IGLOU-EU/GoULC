...
* **AI Disclosure:** Every commit an LLM took part in must carry the trailer `Assisted-by: LLM`, written exactly like that, placed **before** the `Signed-off-by` line so the sign-off stays last. The declaration is mandatory whatever the share of the work, from a reviewed suggestion to a fully generated diff. The form names no vendor, product or model on purpose: a reader needs to know that a machine took part and that a human signed for it, not which company sold the tool. It replaces the former `Co-Authored-By` trailer, which claimed an authorship an LLM cannot hold in law.
